...
def download_pe2(srr, out, done):
    ...
    inputs = []
    outputs = [done]
    options = {'cores': 1, 'memory': '4g', 'walltime': "48:00:00", 'account':"primatediversity"}
...
def download_straggler_old(srr, out, done):
    ...
    inputs = []
    outputs = [done]
    options = {'cores': 1, 'memory': '4g', 'walltime': "24:00:00", 'account':"primatediversity"}
...
def mergeBAMs_custom(bams, output, prev_done, done):
    ...
    outputs = [done]
    # options = default_options.copy()
    options = {"cores": 1, 'memory': "16g", 'walltime': "24:00:00", 'account': "primatediversity"}
    spec = job_header+f"""
    picard MergeSamFiles {" ".join([f"-I {bam} " for bam in bams])}                  \
                         -O {output}                                                 \
                         --SORT_ORDER   queryname                                    \
...
def call_batch_with_bed(group, ind, batch, bed, intervals, ref, ploidy, prev_done, done):
    ...
    outputs = [done]
    # options = default_options.copy()
    options = {"cores": 1, 'memory': "8g", 'walltime': "48:00:00", 'account': "primatediversity"}
