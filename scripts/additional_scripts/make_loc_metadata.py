...
subprocess.run(["rm", "-fr", f"/faststorage/project/primatediversity/data/gVCFs_recalling_10_12_2024/{group}/gVCF/beds_and_intervals/"])
subprocess.run(["mkdir", "-p", f"/faststorage/project/primatediversity/data/gVCFs_recalling_10_12_2024/{group}/gVCF/beds_and_intervals/"])
...
for j in range(len(inds)):
    ...
    for batch in sorted(list(set(regions.batch))):
        ...
        for pl in set_of_ploidies:
            ...
            oo = open(f"/faststorage/project/primatediversity/data/gVCFs_recalling_10_12_2024/{group}/gVCF/beds_and_intervals/{inds[j]}_batch_{batch}_ploidy_{pl}.bed", "w")
            oo.write("\n".join(chromosomes[jj] + "\t" + str(starts[jj] - 1) + "\t" + str(ends[jj]) for jj in range(len(chromosomes))))
            oo.close()

            oo = open(f"/faststorage/project/primatediversity/data/gVCFs_recalling_10_12_2024/{group}/gVCF/beds_and_intervals/{inds[j]}_batch_{batch}_ploidy_{pl}.intervals", "w")
...
for batch in sorted(list(set(regions.batch))):
    ...
    for j in range(set_of_ploidy_pairs.shape[0]):
        ...
        oo = open(f"/faststorage/project/primatediversity/data/gVCFs_recalling_10_12_2024/{group}/gVCF/beds_and_intervals/genDB_{group}_batch_{batch}_fploidy_{female_ploidies[j]}_mploidy_{male_ploidies[j]}.intervals", "w")
        oo.write("\n".join(chromosomes[jj] + ":" + str(starts[jj]) + "-" + str(ends[jj]) for jj in range(len(chromosomes))))
        oo.close()
        ...
        if len(chrs)>0:
            oo = open(f"/faststorage/project/primatediversity/data/gVCFs_recalling_10_12_2024/{group}/gVCF/beds_and_intervals/{group}_batch_{batch}_fploidy_{female_ploidies[j]}_mploidy_{male_ploidies[j]}_loc_{length_of_chunk}_subbatches.intervals", "w")
            oo.write("\n".join(chrs[sb] + ":" + str(sts[sb]) + "-" + str(ens[sb]) for sb in range(len(sts))))
            oo.close()
        
        if len(short_segments)>0:
            oo = open(f"/faststorage/project/primatediversity/data/gVCFs_recalling_10_12_2024/{group}/gVCF/beds_and_intervals/{group}_batch_{batch}_fploidy_{female_ploidies[j]}_mploidy_{male_ploidies[j]}_loc_{length_of_chunk}_short_segments.intervals", "w")
